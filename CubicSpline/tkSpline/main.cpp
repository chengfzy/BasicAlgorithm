...
int main(int argc, char** argv) {
    ...
    for (size_t i = 0; i < X.size(); i++) {
        printf("%f %f %f %f\n", s.m_a[i], s.m_b[i], s.m_c[i], s.m_y[i]);
    }
    printf("\n");
...
/*    for (int i = -50; i < 250; i++) {
        double x = 0.01 * i;
        printf("%f %f %f %f %f\n", x, s(x), s.deriv(1, x), s.deriv(2, x), s.deriv(3, x));
        // checking analytic derivatives and finite differences are close
//        assert(fabs(s.deriv(1, x) - deriv1(s, x)) < 1e-6);
//        assert(fabs(s.deriv(2, x) - deriv2(s, x)) < 1e-6);
    }
    printf("\n");*/
